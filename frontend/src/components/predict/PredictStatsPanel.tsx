...
export function PredictStatsPanel() {
  ...
  if (loading.predictNodes) return <div className="text-sm text-slate-400">{t('common.loading')}</div>;
  ...
  for (const n of predictNodes) {
    ...
    else if (n.risk_score < RISK_THRESHOLD.FRAUD) idx = 3;
    else idx = 4;
    buckets[idx]++;
  }
  const labels = ['[0, 0.2)', '[0.2, 0.4)', '[0.4, 0.6)', `[0.6, ${RISK_THRESHOLD.FRAUD})`, `[${RISK_THRESHOLD.FRAUD}, 1.0]`];
  const colors = ['bg-emerald-500', 'bg-sky-500', 'bg-yellow-500', 'bg-orange-500', 'bg-red-500'];

  return (
    <div className="space-y-4">
      <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
        <span className="w-0.5 h-3.5 bg-violet-500 rounded-full inline-block" aria-hidden />
        {t('predict.title')}
      </h3>

      <div className="grid grid-cols-2 gap-2">
        <div className="bg-slate-700/40 rounded-lg p-2.5 ring-1 ring-slate-600/50">
          <p className="text-xs text-slate-400">{t('predict.summary.total')}</p>
          <p className="text-lg font-bold text-slate-100">{total.toLocaleString()}</p>
        </div>
        <div className="bg-slate-700/40 rounded-lg p-2.5 ring-1 ring-slate-600/50">
          <p className="text-xs text-slate-400">{t('predict.summary.blacklist')}</p>
          <p className="text-lg font-bold text-red-400">{blacklist.toLocaleString()}</p>
        </div>
        <div className="bg-slate-700/40 rounded-lg p-2.5 ring-1 ring-slate-600/50">
          <p className="text-xs text-slate-400">{t('predict.summary.normal')}</p>
          <p className="text-lg font-bold text-emerald-400">{normal.toLocaleString()}</p>
        </div>
        <div className="bg-slate-700/40 rounded-lg p-2.5 ring-1 ring-slate-600/50">
          <p className="text-xs text-slate-400">{t('predict.summary.ratio')}</p>
          <p className="text-lg font-bold text-amber-400">{blacklistRatio}%</p>
        </div>
      </div>

      <div>
        <h4 className="text-xs uppercase tracking-wider text-slate-400 mb-2">{t('risk.distribution')}</h4>
          ...
          {labels.map((label, i) => {
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-slate-700/50 rounded-full h-3 overflow-hidden" aria-hidden>
